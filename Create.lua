...
-- Monitor/Display ist an der rechten Seite: peripheral "right"
-- Controls:
--   1-4  Mode
...
-- Für 6x3 sieht TextScale 2 meistens am besten aus.
-- Wenn Zeichen fehlen oder es nicht 6x3 wirkt: 1, 0.5 oder 3 probieren.
mon.setTextScale(2)
...
local spinner = {"|","/","-","\\"}
local bars = {"▱▱▱","▰▱▱","▰▰▱","▰▰▰"} -- 3-seg bar, wirkt gut auf 6 Zeichen

local function update()
  tick = tick + 1

  if mode == 1 then
    -- idle: smooth pulse 0..100
...
local function draw()
  ...
  local l3 = bar .. tail -- 3 + 2 = 5 chars, wird gepadded auf 6
...
while true do
  ...
  local ev, key = os.pullEventTimeout("key", 0.15)
  if ev == "key" then
